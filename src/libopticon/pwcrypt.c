...
void md5_begin( md5_context *ctx )
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
...
void md5_hash (md5_context *ctx, uint8 *input, uint32 length)
{
    uint32 left, fill;
...
void md5_end (md5_context *ctx, uint8 digest[16])
{
    uint32 last, padn;
    uint32 high, low;
    uint8 msglen[8];
    ...
    md5_hash( ctx, md5_padding, padn );
    md5_hash( ctx, msglen, 8 );
...
static char *md5_pwcrypt(char result[MD5_OUT_BUFSIZE], 
                         unsigned char *pw, unsigned char *salt)
{
    char *p;
    ...
    for (i = pw_len; i; i >>= 1) {
        md5_hash(&ctx, ((i & 1) ? final : (unsigned char *) pw), 1);
    }
    md5_end(&ctx, final);
...
static char MD5CRYPT_BUFFER[40];

char *pwcrypt (const char *pw, const char *salt) {
    char *res = malloc((size_t) 40);
    return md5_pwcrypt (res, (unsigned char *) pw, (unsigned char *) salt);
}
